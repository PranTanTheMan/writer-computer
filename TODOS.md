...
- Global font settings: [`SPECs/global-font-settings-spec.md`](SPECs/global-font-settings-spec.md) — the six per-mode `theme.{mode}.{ui,editor,mono}-font` settings become three global `fonts.{ui,editor,mono}` settings in a "Fonts" section above the theme cards (fonts are typographic, not chromatic); startup migration adopts existing per-mode values (light wins, dark fallback) and drops the old keys. The font control is now a single select-style pill (stack input + chevron in one surface).
